...
static void handleChunk0(int SIZE, hls::burst_maxi<AxiVec4> A,
                         hls::burst_maxi<AxiVec4> B, int c1, int c2) {
  float A_c[3][40];
...
#pragma HLS PIPELINE
    ...
    for (c5 = 0; c5 <= 31; c5 += 1) {
      B_c[0][c5 + 4] =
          0.2 * (A_c[1][c5 + 1 + 4] + A_c[1][c5 + 4] + A_c[1][c5 + 2 + 4] +
                 A_c[2][c5 + 1 + 4] + A_c[0][c5 + 1 + 4]);
    }
    ...
    if (SIZE >= 3 && c3 >= 0 && c3 <= 31 && c1 % 32 == 0 && c2 % 32 == 0) {
      ...
      int leftPad = 0;
      if ((((c2 % 4) == 0) && (((-33) + SIZE) <= c2 && c2 <= ((-3) + SIZE)))) {
        leftPad = 1;
      }
      if ((((c2 % 4) == 0) && (c2 <= ((-34) + SIZE)))) {
        leftPad = 1;
      }
      const int rightPad = (len - payLen) - leftPad;
      const int compLen = len - leftPad;

      B.write_request((c2 + (-1) + 1 + ((c1 + c3 + 1) * SIZE)) / 4, len / 4);

      for (int cc1 = -1; cc1 <= 31; cc1 += 4) {
        if (cc1 < compLen) {
          AxiVec4 t{B_c[0][cc1 + 4], B_c[0][cc1 + 5], B_c[0][cc1 + 6],
                    B_c[0][cc1 + 7]};
