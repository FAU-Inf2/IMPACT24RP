...
#pragma HLS ARRAY_PARTITION variable = HZ_c type = complete dim = 2
  ...
  if (SIZEX >= 2 && SIZEY >= 1 && c3 == 0 && c1 % 64 == 0 && c2 % 64 == 0) {
    int len = 0;
    if ((((-48) + SIZEY) <= c2 && c2 < SIZEY)) {
      len =
...
#pragma HLS PIPELINE
    ...
    if (SIZEX >= 2 && SIZEY >= 1 && c3 >= 0 && c3 <= 63 && c1 % 64 == 0 &&
        c2 % 64 == 0) {
      int len = 0;
      if ((c2 <= ((-50) + SIZEY))) {
        len = ((64 + (16 * (floor(c2 / 16)))) - (16 * (floor((15 + c2) / 16))));
      }
      if ((((-49) + SIZEY) <= c2 && c2 < SIZEY)) {
