...
#pragma HLS ARRAY_PARTITION variable = EY_c type = complete dim = 2
  ...
  if (SIZEX >= 2 && SIZEY >= 2 && c3 == 0 && c1 % 64 == 0 && c2 % 64 == 0) {
    int len = 0;
    if ((((-49) + SIZEY) <= c2 && c2 <= ((-2) + SIZEY))) {
      len =
          ((16 * (floor((14 + SIZEY) / 16))) - (16 * (floor((15 + c2) / 16))));
    }
    if ((c2 <= ((-50) + SIZEY))) {
      len = ((64 + (16 * (floor(c2 / 16)))) - (16 * (floor((15 + c2) / 16))));
    }
  ...
  for (c3 = 0; c3 <= min(63, SIZEX - c1 - 2); c3 += 1) {
#pragma HLS PIPELINE

        int c5 = 0;

    // Cache load for HZ at dim j
    if (SIZEX >= 2 && SIZEY >= 2 && c3 >= 0 && c3 <= 63 && c1 % 64 == 0 &&
        c2 % 64 == 0) {
      int len = 0;
      if ((c2 <= ((-51) + SIZEY))) {
        len = ((64 + (16 * (floor(c2 / 16)))) - (16 * (floor((15 + c2) / 16))));
      }
      if ((((-50) + SIZEY) <= c2 && c2 <= ((-2) + SIZEY))) {
