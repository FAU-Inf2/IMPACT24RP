...
#pragma HLS ARRAY_PARTITION variable = B_c type = complete dim = 1
  ...
  if (SIZE >= 3 && c1 % 1024 == 0) {
    ...
    if ((c1 <= ((-1025) + SIZE))) {
      len = ((1040 + (16 * (floor(c1 / 16)))) - (16 * (floor((15 + c1) / 16))));
    }

    int leftPad = 0;
    if ((((c1 % 16) == 0) && (((-1025) + SIZE) <= c1 && c1 <= ((-2) + SIZE)))) {
      leftPad = 1;
    }
    if ((((c1 % 16) == 0) && (c1 <= ((-1026) + SIZE)))) {
      leftPad = 1;
    }

    const int compLen = len - leftPad;

    A.read_request((c1 + (-1) + 1) / 16, len / 16);

    for (int cc0 = -1; cc0 <= 1024; cc0 += 16) {
      if (cc0 < compLen) {
  ...
  for (c2 = 0; c2 <= 1023; c2 += 1) {
    B_c[c2 + 16] =
        0.33333 * (A_c[c2 + 1 + 16] + A_c[c2 + 16] + A_c[c2 + 1 + 16]);
  }

  // write back for arr B
  if (SIZE >= 3 && c1 % 1024 == 0) {
    int len = 0;
    if ((((-1025) + SIZE) <= c1 && c1 <= ((-2) + SIZE))) {
    ...
    if ((((-1026) + SIZE) <= c1 && c1 <= ((-3) + SIZE))) {
      payLen = (((-2) + SIZE) - c1);
    }
    int leftPad = 0;
    if ((((c1 % 16) == 0) && (((-1025) + SIZE) <= c1 && c1 <= ((-3) + SIZE)))) {
      leftPad = 1;
    }
    if ((((c1 % 16) == 0) && (c1 <= ((-1026) + SIZE)))) {
      leftPad = 1;
    ...
    for (int cc0 = -1; cc0 <= min(1023, SIZE - c1 - 3); cc0 += 16) {
      if (cc0 < compLen) {
...
void tlf(int SIZE, hls::burst_maxi<AxiVec16> A, hls::burst_maxi<AxiVec16> B) {
  // partition
  for (int c1 = 0; c1 < SIZE - 2; c1 += 1024) {
