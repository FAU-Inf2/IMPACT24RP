...
static void handleChunk0(int SIZEX, int SIZEY, hls::burst_maxi<AxiVec16> EX,
...
#pragma HLS ARRAY_PARTITION variable = HZ_c type = complete dim = 2
  ...
  for (c3 = 0; c3 <= min(127, SIZEX - c1 - 1); c3 += 1) {
#pragma HLS PIPELINE

        int c5 = 0;

    // Cache load for EX at dim j
    if (SIZEX >= 1 && SIZEY >= 2 && c3 >= 0 && c3 <= 127 && c1 % 128 == 0 &&
        c2 % 128 == 0) {
      int len = 0;
      if ((c2 <= ((-130) + SIZEY))) {
        len =
      ...
      int leftPad = 0;
      if ((((c2 % 16) == 0) &&
           (((-128) + SIZEY) <= c2 && c2 <= ((-2) + SIZEY)))) {
        leftPad = 1;
      }
      if ((((c2 % 16) == 0) && (c2 <= ((-129) + SIZEY)))) {
        leftPad = 1;
      }

      const int compLen = len - leftPad;

      EX.read_request((c2 + (-1) + 1 + ((c1 + c3) * SIZEY)) / 16, len / 16);

      for (int cc1 = -1; cc1 <= 127; cc1 += 16) {
        if (cc1 < compLen) {
    ...
    for (c5 = 0; c5 <= min(127, SIZEY - c2 - 2); c5 += 1) {
      EX_c[0][c5 + 16] =
          EX_c[0][c5 + 16] - 0.5 * (HZ_c[0][c5 + 1 + 16] - HZ_c[0][c5 + 16]);
    }
    ...
    if (SIZEX >= 1 && SIZEY >= 2 && c3 >= 0 && c3 <= 127 && c1 % 128 == 0 &&
      ...
      EX.write_request((c2 + (-1) + 1 + ((c1 + c3) * SIZEY)) / 16, len / 16);

      for (int cc1 = -1; cc1 <= 127; cc1 += 16) {
        if (cc1 < compLen) {
          AxiVec16 t{EX_c[0][cc1 + 16], EX_c[0][cc1 + 17], EX_c[0][cc1 + 18],
                     EX_c[0][cc1 + 19], EX_c[0][cc1 + 20], EX_c[0][cc1 + 21],
                     EX_c[0][cc1 + 22], EX_c[0][cc1 + 23], EX_c[0][cc1 + 24],
                     EX_c[0][cc1 + 25], EX_c[0][cc1 + 26], EX_c[0][cc1 + 27],
                     EX_c[0][cc1 + 28], EX_c[0][cc1 + 29], EX_c[0][cc1 + 30],
                     EX_c[0][cc1 + 31]};
...
void tlf(int SIZEX, int SIZEY, hls::burst_maxi<AxiVec16> EX,
         hls::burst_maxi<AxiVec16> HZ) {
  // partition
  for (int c1 = 0; c1 < SIZEX; c1 += 128) {
