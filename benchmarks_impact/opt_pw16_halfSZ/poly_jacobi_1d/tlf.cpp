...
#pragma HLS ARRAY_PARTITION variable = B_c type = complete dim = 1
  ...
  if (SIZE >= 3 && c1 % 256 == 0) {
    ...
    if ((c1 <= ((-257) + SIZE))) {
      len = ((272 + (16 * (floor(c1 / 16)))) - (16 * (floor((15 + c1) / 16))));
    }
  ...
  if (SIZE >= 3 && c1 % 256 == 0) {
    ...
    if ((((-258) + SIZE) <= c1 && c1 <= ((-3) + SIZE))) {
      payLen = (((-2) + SIZE) - c1);
    }
    int leftPad = 0;
    if ((((c1 % 16) == 0) && (((-257) + SIZE) <= c1 && c1 <= ((-3) + SIZE)))) {
      leftPad = 1;
    }
    if ((((c1 % 16) == 0) && (c1 <= ((-258) + SIZE)))) {
      leftPad = 1;
    }
