...
Res<float, 2> driver_fpga(int SIZEY, int SIZEX, unsigned int REPS) {
...
#pragma omp target orkaTranslate(topLevelFunction                              \
                                 : "tlf") map(tofrom                           \
                                              : a0[:a0_size], a1               \
                                                  [:a1_size])
#endif
  { tlf(SIZEX, SIZEY, a0, a1); }
  clock_gettime(CLOCK, &end);
  elapsed_time_hr(start, end, "FPGA code");
  return {A0, A1};
}

Res<float, 2> driver_host(int SIZEY, int SIZEX, unsigned int REPS) {
  std::vector<int> dims0{SIZEX, SIZEY};
