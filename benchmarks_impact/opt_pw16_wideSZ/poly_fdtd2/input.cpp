...
Res<float, 3> driver_fpga(int SIZEX, int SIZEY, unsigned int REPS) {
  std::vector<int> dims0{SIZEX, SIZEY};
  auto A0 = getAndInitAxiArray<AxiVec16>(dims0, 0, "a0");
  AxiVec16 *a0 = A0->getCastedPtr<AxiVec16>();
  const int a0_size = A0->getOmpMapSize();
  std::vector<int> dims1{SIZEX, SIZEY};
  auto A1 = getAndInitAxiArray<AxiVec16>(dims1, 0, "a1");
  AxiVec16 *a1 = A1->getCastedPtr<AxiVec16>();
  const int a1_size = A1->getOmpMapSize();
  std::vector<int> dims2{SIZEX, SIZEY};
  auto A2 = getAndInitAxiArray<AxiVec16>(dims2, 0, "a2");
  AxiVec16 *a2 = A2->getCastedPtr<AxiVec16>();
...
Res<float, 3> driver_host(int SIZEX, int SIZEY, unsigned int REPS) {
  std::vector<int> dims0{SIZEX, SIZEY};
